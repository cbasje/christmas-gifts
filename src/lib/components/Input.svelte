...
const labelClass =
	'block text-sm font-medium text-gray-600 dark:text-gray-300 ' + ($$props['label-class'] ?? '');
const inputClass =
...
const messageClass =
	...
	($$props['message-class'] ?? '');
const helpClass =
	'mt-1 block w-full text-sm text-gray-400 dark:text-gray-500 ' + ($$props['help-class'] ?? '');
</script>
