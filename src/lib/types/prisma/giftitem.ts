...
export const GiftItemSchema = z.object({
  id: z.string().uuid(),
  name: z.string(),
  price: z.string().regex(/(?:[$€])?\s?\d+(?:[,.]\d+)?/g, { message: "Price must consist of numbers with currency codes." }).nullish(),
 ...
 */
export const RelatedGiftItemSchema: z.ZodSchema<CompleteGiftItem> = z.lazy(() => GiftItemSchema.extend({
